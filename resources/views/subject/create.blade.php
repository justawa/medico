              ...
              <div class="card-body">
                <div class="form-group @error('course') has-danger @enderror">
                    ...
                    @foreach($courses as $course)
                        <option @if(old('course', $subject->course_id) == $course->id) selected @endif value="{{ $course->id }}">{{ $course->name }}</option>
                        @endforeach
                ...
                <div class="form-group @error('name') has-danger @enderror">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('subject', $subject->name) }}" placeholder="Subject Name" required>
                    @error('name') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>

                <div class="form-group @error('summary') has-danger @enderror">
                    <label class="col-form-label" for="summary">Summary</label>
                    <textarea class="form-control @error('summary') is-invalid @enderror" cols="30" rows="8" placeholder="Subject Summary" id="summary" name="summary" >{{ old('subject', $subject->summary) }}</textarea>
